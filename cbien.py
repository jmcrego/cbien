...
def do_sentence_vectors(args):
    ...
    with torch.no_grad():
        ...
        nsent = 0
        ntoks = 0
        tstart = arrow.utcnow() #datetime.now()
        ...
        tend = arrow.utcnow() #datetime.now()
        telapsed = 1000*(tend-tstart)
        logging.info('processed {} sentences, {} tokens, in {} ms'.format(nsent,ntoks,telapsed))
